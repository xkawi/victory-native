...
const Helpers = {
  ...
  onTouchEnd() {
    return [{
      target: "parent",
      mutation: () => {
        return { panning: false, originalPinchDistance: null, zoomActive: false };
      }
    }];
  },
  onTouchPinch(evt, targetProps, eventKey, ctx) { // eslint-disable-line max-params
    const { onDomainChange, zoomDimension, zoomDomain } = targetProps;
    ...
    return [{
      ...
      mutation: () => {
        return {
          ...
          parentControlledProps: ["domain"], panning: false, originalPinchDistance, zoomActive
        };
      }
    }];
  },
